...
const content = `import { Component, inject, signal, computed, OnInit } from '@angular/core';
...
@Component({
        ...
        <div class="card tier-map">
          ...
          <div class="tier-steps">
            ...
            <div class="ts-item" [class.ts-active]="tierLevel() >= 4" [class.ts-current]="tierLevel() === 4">
              <div class="ts-badge" style="background:linear-gradient(135deg,#7c3aed,#4f46e5)">💎</div>
  ...
  styles: [\`
    ...
    .ts-item.ts-current .ts-label { color:var(--primary,#3949ab); font-weight:700; }
    ...
    .ts-line.ts-done { background:#3949ab; }
    ...
    .redeem-card:hover:not(.disabled) { border-color:var(--primary,#3949ab); transform:translateY(-2px); box-shadow:0 4px 12px rgba(57,73,171,0.15); }
    .redeem-card.selected { border-color:var(--primary,#3949ab); background:#f0f4ff; }
    .redeem-card.disabled { opacity:0.4; cursor:not-allowed; }
    .rc-pts   { font-size:1.1rem; font-weight:800; color:var(--primary,#3949ab); }
...
export class RewardsComponent implements OnInit {
  ...
  tierBg = computed(() => {
    switch (this.tierLevel()) {
      case 4: return 'linear-gradient(135deg,#7c3aed,#4f46e5)';
...
const outPath = path.join(__dirname, 'frontend/src/app/pages/rewards/rewards.component.ts');
fs.writeFileSync(outPath, content, 'utf8');
console.log('rewards.component.ts written:', content.length, 'bytes');
