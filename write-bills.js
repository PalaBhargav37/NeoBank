...
const content = `import { Component, OnInit, signal, computed, inject } from '@angular/core';
      ...
      @if (step() === 4 && lastReceipt()) {
        <div class="card step-card receipt-card">
          ...
          <div class="receipt-pts-note">
            🎁 You earned <strong>{{ lastReceipt().amount | number:'1.0-0' | rewardPoints }}+ reward points</strong> for this payment!
          </div>
      ...
      next: r => {
        ...
        if (r.success) {
          ...
          this.lastReceipt.set({
            ...
            time:             new Date(),
            txId:             r.data?.transactionId || r.data?.id || ('TXN' + Date.now().toString().slice(-8))
          });
...
const outPath = path.join(__dirname, 'frontend/src/app/pages/bills/bills.component.ts');
fs.writeFileSync(outPath, content, 'utf8');
console.log('bills.component.ts written:', content.length, 'bytes');
